...
def sms_send(a,msg):
    ...
    params={
        "authorization":"yCO8PF6ADJuK9tgnhMxYfwpQZmdGvbN0X3rBI1k5L74jsSiaeowRgjiZG2NnB5fILEbFX78OJT4M1t3q",
        "sender_id":"SMSINI",
        "message":msg,
        "language":"english",
        "route":"p",
...
def check():
    ...
    if(c==""):
        ...
        if(str(rand)==c):
            tsmg.showinfo("Info","Verification Successful")
        else:
            tsmg.showerror("Error","Invalid OTP")
            num.set("")
            otp.set("")
root.geometry("500x500")
root.title("OTP VERFICATION SYSTEM")
num=StringVar()
otp=StringVar()
f1=Frame(root)
Label(f1,text="OTP VERFICATION",font="SegoeUI 30 bold",fg="purple").pack(padx=5,pady=10)
f1.pack(fill=BOTH)
f2=Frame(root)
